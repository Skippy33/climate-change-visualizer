...
def FindClimateData(latitude, longitude):# Finds climate data based on Lat and Long
    ...
    #convert totalweatherdata to a series
    totalweatherdata = pandas.Series(totalweatherdata)
    #find the MAD
    MAD = 3.5 * totalweatherdata.mad()

    for datapoint in totalweatherdata:#for each datapoint in totalweatherdata
        if datapoint != datapoint:#if it's a nan, continue
            continue
        if abs(totalweatherdata.mean()-datapoint) > MAD:#if the absolute difference from the mean is more than 3x the MAD
            #replace the erroneous data with a NAN
            totalweatherdata.replace(to_replace=datapoint, value=numpy.nan, inplace=True)

    #change the lat/long to the correct location
...
def Infopage(years, ysmoothed, rawy, latitude, longitude):# makes the page to display info
    ...
    try:
        ...
        infolabel = tk.Label(infopage, pady=30, padx=10, width=100, text="Y axis does not start from 0.\n"
        "X axis ends at the current year. Orange line is raw yearly average, blue is smoothed\nBroken lines show a break in the weatherkeeping records. Try a larger city if this issue is too bad\n\n"
        "\nExact location of weather station: " + str(geolocator.reverse(str(latitude) + ",  " + str(longitude)).raw["address"]["aeroway"]))
        print(str(geolocator.reverse(str(latitude) + ",  " + str(longitude)).raw["address"]))
    #set the font
